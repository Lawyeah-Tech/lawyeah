---
name: 评估房屋租赁争议
description: 效力、履行抗辩、解除路径、押金租金区间
---
...
形成房屋租赁已成争的效力、履行抗辩、解除路径和押金租金区间。C 端租赁解约只分流。动产租赁转一般合同。

`depends-on` 为空。接受用户、律师、机关或其他来源的等价事实、测算和证据清单。

## 工作边界

正触发：租金、维修、转租、提前解约已成争。
...
确认：租赁合同期限与用途；备案；转租；欠租期间；维修义务约定；押金。
...
宿主可调用时：`search_law` 核验民法典租赁合同章及城镇房屋租赁合同解释（2020 修正）。
...
动产租赁转一般合同。未经消防或规划的违法建筑租赁效力待核。

### 节点 2　效力与备案

未备案一般不影响效力。一房数租履行顺序另转专项。
...
租金、占有使用费、押金未知不写零。C 端只分流。

## 停止与安全

- `整体停止`：用户坚持停水停电逼迁或伪造欠租。
- `字段停止`：租赁效力或解除规则未核验。
- `目标停止`：主目标变为 C 端解约包、动产租赁或发函。
