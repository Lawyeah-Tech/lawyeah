---
name: 分流污染环境罪刑民交叉
description: 分流污染环境刑民行程序并列出必须停止的事项
---
...
在明确、无利益冲突的单方立场下，形成“污染环境罪刑民交叉分流包”：刑民行边界、停止项、报案或民事并行提示。不定罪。辩护或控告转刑事。

`depends-on` 为空。接受用户、律师、机关或其他来源的等价事实、测算和证据清单。

## 工作边界

正触发：出现污染环境罪、非法处置危废、伪造监测数据迹象。
...
宿主可调用时：search_law 核验刑法污染环境罪及环境监测数据造假相关罪立案追诉标准（红旗）、刑事诉讼法与民事并行规则接口。 不虚构案件系统。核验重点：刑法及相关司法解释立案追诉标准（红旗）、环境保护行政执法与刑事司法衔接、民事诉讼法并行接口。
...
- `整体停止`：串供、毁证或指导继续犯罪；利益冲突未解；基础主体无法建立。
- `字段停止`：立案追诉标准、并行规则未核验；法域、日期、文书送达、动态规则未核验。
- `目标停止`：主目标变为刑事辩护或控告。
