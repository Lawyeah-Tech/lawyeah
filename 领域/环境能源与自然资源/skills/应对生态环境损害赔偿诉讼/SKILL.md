---
name: 应对生态环境损害赔偿诉讼
description: 准备生态环境损害赔偿诉讼的答辩、鉴定异议和修复替代
---
...
在明确、无利益冲突的被告立场下，形成“生态环境损害赔偿诉讼应诉包”：答辩、鉴定异议、修复替代。磋商阶段不写应诉终稿。
...
宿主可调用时：search_law 核验生态损害赔偿诉讼抗辩、磋商前置抗辩、修复替代、鉴定异议接口。 不虚构案件系统。核验重点：生态环境损害赔偿管理规定、民法典第 1234–1235 条、鉴定异议规则。
...
- `整体停止`：转移财产或破坏修复现场；利益冲突未解；基础主体无法建立。
- `字段停止`：前置规则、请求范围未核验；法域、日期、文书送达、动态规则未核验。
- `目标停止`：主目标变为磋商方案或另案应诉。
