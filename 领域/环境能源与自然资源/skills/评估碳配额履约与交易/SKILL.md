---
name: 评估碳配额履约与交易
description: 判断碳市场纳入、配额清缴和交易效力，不预测碳价
---
...
在明确、无利益冲突的控排单位立场下，形成“碳配额履约与交易矩阵”：纳入、配额、清缴、交易效力、处罚接口。不预测碳价。

`depends-on` 为空。接受用户、律师、机关或其他来源的等价事实、测算和证据清单。

## 工作边界

正触发：控排企业配额缺口、清缴或交易发生争议。
...
确认：使用者与授权；单方立场（控排单位）；相对方、主管机关及可能责任主体；关键文本版本；法域；当前检查、处罚、协商、诉讼或执行状态；机关文书作出与送达；对方已知主张；关键材料及缺失。
...
宿主可调用时：search_law 核验碳排放权交易管理暂行条例、登记结算交易规则、清缴和处罚。 不虚构案件系统。核验重点：碳排放权交易管理暂行条例、配套登记交易结算规则。
...
- `整体停止`：伪造排放数据；利益冲突未解；基础主体无法建立。
- `字段停止`：条例、清缴截止、登记规则未核验；法域、日期、文书送达、动态规则未核验。
- `目标停止`：主目标变为预测碳价或核查造假。
