---
name: 评估突发环境事件预案与演练
description: 判断突发环境事件预案、备案和演练缺口
---
...
在明确、无利益冲突的单位立场下，形成“突发环境事件预案与演练缺口矩阵”：预案义务、风险评估、备案、演练、隐患排查和与事故报告的分界。

`depends-on` 为空。接受用户、律师、机关或其他来源的等价事实、测算和证据清单。

## 工作边界

正触发：预案未备案、未演练、风险评估缺失或专项预案过期，单位需要义务缺口判断。

负触发并输出 `触发判定：负触发，未启动本任务`：

- 已发生或可能正在发生的事故要发送版报告（转 P0 报告原子）；
- 指导事后补造演练照片或倒签备案；
- 只要总体总图；
...
确认：使用者与授权；单方立场（生产经营单位）；相对方、主管机关及可能责任主体；关键文本版本；法域；当前检查、处罚、协商、诉讼或执行状态；机关文书作出与送达；对方已知主张；关键材料及缺失。
...
宿主可调用时：search_law 核验环境保护法应急条款、突发环境事件应急管理办法、预案备案层级；事故报告另项。 不虚构案件系统。核验重点：环境保护法、突发环境事件应急管理办法、国家突发环境事件应急预案接口。
...
- `整体停止`：立场不明；用户坚持倒签或假演练；利益冲突未解；基础主体无法建立。
- `字段停止`：备案规则、演练频次未核验；法域、日期、文书送达、动态规则未核验。
- `目标停止`：主目标变为事故报告终稿。
