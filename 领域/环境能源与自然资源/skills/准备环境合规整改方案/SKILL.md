---
name: 准备环境合规整改方案
description: 形成可核验的环境合规整改方案，不保证免罚
---
...
在明确、无利益冲突的排污单位立场下，形成可核验的“环境合规整改方案”：整改事项、合法措施、时限、验收证据、不得虚假整改的边界。

`depends-on` 为空。接受用户、律师、机关或其他来源的等价事实、测算和证据清单。

## 工作边界

正触发：检查、处罚或督察后需要可核验整改方案，排污单位尚未要求本任务写申辩终稿。
...
宿主可调用时：search_law 核验责令改正、限制生产、停产整治措施边界、验收或复查规则；不检索虚假整改方法。 不虚构案件系统。核验重点：环境保护法责令改正、生态环境行政处罚办法、相关行业整改与验收规范接口。
...
- `整体停止`：立场不明；用户坚持虚假整改；利益冲突未解；基础主体无法建立。
- `字段停止`：责令改正依据、时限、验收规则未核验；法域、日期、文书送达、动态规则未核验。
- `目标停止`：主目标变为申辩终稿或假验收。
