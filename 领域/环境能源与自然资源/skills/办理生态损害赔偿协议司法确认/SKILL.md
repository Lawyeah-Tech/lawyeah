---
name: 办理生态损害赔偿协议司法确认
description: 办理生态环境损害赔偿协议司法确认并明示无确认则无执行力
---
...
在明确、无利益冲突的单方立场下，形成“生态环境损害赔偿协议司法确认路径包”：路径、材料、失败分流。单独协议无执行力必须明示。

`depends-on` 为空。接受用户、律师、机关或其他来源的等价事实、测算和证据清单。

## 工作边界

正触发：需司法确认增强生态环境损害赔偿协议执行力。
...
宿主可调用时：search_law 核验生态环境损害赔偿协议司法确认相关规则、民事诉讼法特别程序接口。 不虚构案件系统。核验重点：生态环境损害赔偿协议司法确认规则、民事诉讼法司法确认接口。
...
- `整体停止`：伪造签署或公示；利益冲突未解；基础主体无法建立。
- `字段停止`：司法确认规则未核验；法域、日期、文书送达、动态规则未核验。
