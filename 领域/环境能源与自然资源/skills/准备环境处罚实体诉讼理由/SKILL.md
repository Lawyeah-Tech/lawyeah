---
name: 准备环境处罚实体诉讼理由
description: 整理环境处罚或许可的实体诉讼理由，程序转行政
---
...
在明确、无利益冲突的当事人立场下，形成“环境处罚或许可实体诉讼理由包”：实体请求、合法性争点、证据，程序问题转行政领域。

`depends-on` 为空。接受用户、律师、机关或其他来源的等价事实、测算和证据清单。

## 工作边界

正触发：已决定就环境处罚或许可提起行政诉讼，只要实体理由。

负触发并输出 `触发判定：负触发，未启动本任务`：

- 起诉期限计算、管辖、立案格式、庭审程序（转行政）；
- 民事环境污染起诉（转私益起诉原子）；
- 决定前申辩；
- 矿业公司股权并购争议整案（转投资并购）；
...
确认：使用者与授权；单方立场（原告（被处罚人或许可申请人等））；相对方、主管机关及可能责任主体；关键文本版本；法域；当前检查、处罚、协商、诉讼或执行状态；机关文书作出与送达；对方已知主张；关键材料及缺失。
...
宿主可调用时：search_law 核验环保实体合法性、许可条件、裁量明显不当；行政诉讼法程序条款不展开。 不虚构案件系统。核验重点：环境保护实体法、行政许可实体条件、行政处罚法实体条款；行政诉讼法程序转行政。
...
- `整体停止`：用户坚持虚假陈述；利益冲突未解；基础主体无法建立。
- `字段停止`：实体规则未核验；法域、日期、文书送达、动态规则未核验。
