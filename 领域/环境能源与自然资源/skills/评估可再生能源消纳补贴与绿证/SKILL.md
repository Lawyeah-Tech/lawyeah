---
name: 评估可再生能源消纳补贴与绿证
description: 判断可再生能源补贴资格、绿证和消纳责任
---
...
在明确、无利益冲突的单方立场下，形成“可再生能源消纳、补贴与绿证矩阵”：补贴资格、绿证、消纳权重、请求路径。不保证财政拨款时点。

`depends-on` 为空。接受用户、律师、机关或其他来源的等价事实、测算和证据清单。

## 工作边界

正触发：补贴拖欠、绿证或消纳责任发生争议。
...
确认：使用者与授权；单方立场（发电企业或消纳义务主体（强制单方））；相对方、主管机关及可能责任主体；关键文本版本；法域；当前检查、处罚、协商、诉讼或执行状态；机关文书作出与送达；对方已知主张；关键材料及缺失。
...
宿主可调用时：search_law 核验可再生能源法、补贴目录规则、可再生能源绿色电力证书、消纳责任权重。 不虚构案件系统。核验重点：可再生能源法、绿证和消纳责任相关办法（须核验现行文本）。
...
- `整体停止`：伪造电量或绿证；利益冲突未解；基础主体无法建立。
- `字段停止`：补贴、绿证、消纳规则未核验；法域、日期、文书送达、动态规则未核验。
- `目标停止`：主目标变为碳履约或纯合同诉讼。
