---
name: 应对生态环境案件执行
description: 提出生态环境执行的适格异议和分期修复方案
---
...
在明确、无利益冲突的被执行人立场下，形成“生态环境案件执行应对包”：适格异议、分期修复方案。禁止隐匿转移、破坏修复现场。
...
确认：使用者与授权；单方立场（被执行人）；相对方、主管机关及可能责任主体；关键文本版本；法域；当前检查、处罚、协商、诉讼或执行状态；机关文书作出与送达；对方已知主张；关键材料及缺失。
...
宿主可调用时：search_law 核验执行异议、暂缓或分期履行、行为执行、拒不执行判决裁定红旗。 不虚构案件系统。核验重点：民事诉讼法执行异议、拒不执行判决裁定罪红旗、环境修复执行实践规则。
...
- `整体停止`：隐匿转移财产或破坏修复现场；利益冲突未解；基础主体无法建立。
- `字段停止`：执行异议规则未核验；法域、日期、文书送达、动态规则未核验。
- `目标停止`：主目标变为重审实体或拒执对抗。
