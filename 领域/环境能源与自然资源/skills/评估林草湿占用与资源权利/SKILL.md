---
name: 评估林草湿占用与资源权利
description: 判断林地草地湿地占用许可、补偿和恢复义务
---
...
在明确、无利益冲突的单方立场下，形成“林草湿占用与资源权利矩阵”：许可、补偿、恢复、权利冲突。农村承包流转只分流；房地开发主轴转房地产。

`depends-on` 为空。接受用户、律师、机关或其他来源的等价事实、测算和证据清单。

## 工作边界

正触发：占用林地草地湿地、采伐或流转需要权利与许可判断。
...
确认：使用者与授权；单方立场（使用权人或建设单位（强制单方））；相对方、主管机关及可能责任主体；关键文本版本；法域；当前检查、处罚、协商、诉讼或执行状态；机关文书作出与送达；对方已知主张；关键材料及缺失。
...
宿主可调用时：search_law 核验森林法、草原法、湿地保护法占用征收、采伐、恢复；承包流转只分流。 不虚构案件系统。核验重点：森林法、草原法、湿地保护法、林地审核审批管理办法接口。
...
- `整体停止`：指导毁林开垦或填占湿地；利益冲突未解；基础主体无法建立。
- `字段停止`：林草湿许可规则、地类认定未核验；法域、日期、文书送达、动态规则未核验。
- `目标停止`：主目标变为承包流转或房地开发。
