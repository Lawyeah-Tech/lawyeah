---
name: 准备碳排放报告核查异议
description: 整理碳排放核查或配额分配异议点与复核路径
---
...
在明确、无利益冲突的控排单位立场下，形成“碳排放报告核查异议包”：异议点、数据、复核路径。不伪造排放数据。
...
宿主可调用时：search_law 核验碳排放权交易管理暂行条例核查、复核、配额申诉接口。 不虚构案件系统。核验重点：碳排放权交易管理暂行条例、核查技术指南接口、配额分配方案。
...
- `整体停止`：伪造或改排放数据；利益冲突未解；基础主体无法建立。
- `字段停止`：核查复核规则未核验；法域、日期、文书送达、动态规则未核验。
- `目标停止`：主目标变为交易获利策略。
