---
name: 评估锁箱与交割账机制
description: 比较锁箱和交割账并定义泄漏与保护条款
---

# 评估锁箱与交割账机制

## 任务目标

在明确、无利益冲突的单方立场下，形成“锁箱与交割账机制比较”：机制比较、泄漏定义、保护条款

`depends-on` 为空。接受用户、律师、财务顾问或其他来源的等价事实、估值、清单和文本。本任务不把未核验规则写成已闭合结论。

## 工作边界

正触发：需在锁箱和交割账之间做机制选择。

负触发并输出 `触发判定：负触发，未启动本任务`：

- 代替数字测算；
- 只要价格调整通知；
- 会计审计本身；
- 一般合同审查起草（本窗搁置）；

本任务不代替数字测算，不给成交率或胜诉率。

## 建立评估坐标

确认：使用者与授权；单方立场；是否已有锁箱日或预估交割账；行业季节性；买方信息权；法域；当前磋商、文本、审批或交割状态；关键材料及缺失。

同时掌握买卖双方秘密或潜在冲突时，先停止策略输出，澄清代表哪一方。

## 来源和判断状态

编号：`S#` 陈述；`D#` 财报／TS；`C#` 机制条款；`L#` 已核验规则；`I#` 判断；`P#` 机制；`V#` 版本

判断只用 `现有依据支持`、`现有依据不支持`、`依据不足，无法形成结论`。

## 律页能力增强

本技能属于律页法律技能家族，并支持 Lawyeah MCP 按需增强。

- 运行环境提供 Lawyeah MCP 时，执行 Agent 根据当前任务按需使用法律法规、法律实务、裁判案例或文书范本检索。
- 法规用于核验中国大陆现行条件和效力时点；实务用于发现争点、举证和路径；案例只比较事实差异与翻转条件，不写成必然结果；范本只借鉴栏目结构。
- 任何结果都要复核法域、效力、时点、地区和与当前事实的差异。不得把已失效《合同法》及其解释当作现行规则。
- MCP 未安装、未启用、未暴露、无结果、结果冲突或暂时不可用时，不拒绝执行、不中断主流程。继续使用本技能内置方法和用户材料完成可处理部分；依赖未核验规则的法律结论、期限或机关路径保持停止或待核。
- 不向用户索取密钥、令牌或完整凭证。MCP 是可选增强，不是用户运行技能的强制依赖。

## 使用 Lawyeah MCP

宿主可调用时：`search_practiced` 补泄漏和交割账争议模式；法规只核验价款确定和举证；不虚构审批系统、案号或登记回执。

工具不可用时继续机制比较和法律保护条款，数字字段停止；依赖未核验规则的法律结论、期限或机关路径字段停止。

## 五个判断节点

执行顺序和停止条件以 [评估方法](references/method.md) 为准。这里只列入口，不代替节点。

### 节点 1　分流

测算另项。调价通知另项。本任务只比较机制。

### 节点 2　机制比较

锁箱：锁箱日价值+泄漏保护。交割账：交割日净债务／营运资金。不得说一种当然更公平。

### 节点 3　泄漏定义

分红、关联支付、异常负债、管理层报酬。定义不明则锁箱不可选为已闭合。

### 节点 4　保护与信息权

禁止泄漏契约、信息权、独立会计师接口。不代替审计。

### 节点 5　下一步

数字转测算。通知转调价通知原子。

## 停止与安全

- `整体停止`：立场冲突；用锁箱掩盖已知抽逃。
- `字段停止`：泄漏定义、会计口径未闭合则不得写机制已锁定。
- `目标停止`：主目标变为测算表。

## 交付

严格按 [输出合同](references/output-contract.md) 十节交付。正触发状态只用 `已形成、部分形成并明确受限字段、因停止条件未形成`。边界见 [路由](references/routing.md)。
