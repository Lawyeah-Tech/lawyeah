---
name: 测算退税与抵缴
description: 列出多缴、留抵或结算退抵条件、所属期和证据，不保证退还
---
...
确认：授权；退抵类型；所属期；缴款书；留抵明细；机关是否已否决。
...
宿主可调用时：`search_law` 核验征管法第五十一条、增值税留抵退税、结算退税现行规则。

工具不可用时继续事实、主体、程序文书状态和问题清单；法律结论、期限或机关路径字段停止。

## 五个判断节点

执行顺序和停止条件以 [退税与抵缴测算方法](references/method.md) 为准。这里只列入口，不代替节点。

### 节点 1　类型

多缴退税、误收退税、减免退税、留抵退税、汇算结算分列。
...
缴款书、申报表、留抵勾选。未知金额不写零。

### 节点 5　与赔偿分界

违法保全造成损失走行政赔偿 P2，本任务不替代。

## 停止与安全

- `整体停止`：虚构多缴或虚假留抵。
- `字段停止`：第五十一条、留抵退税条件、申请期限未核验。
- `目标停止`：出口骗税风险或保证退还。
