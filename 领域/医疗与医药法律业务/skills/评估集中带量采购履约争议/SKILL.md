---
name: 评估集中带量采购履约争议
description: 评估集采断供、质量与量价协议争点及监管交叉
---
...
在明确、无利益冲突的企业或医疗机构立场下，形成“协议争点、违约、监管交叉”。

`depends-on` 为空。接受用户、律师、医学专家、机关或其他来源的等价事实、病历、鉴定、许可和费用清单。

## 工作边界

正触发：集采断供、质量或量价协议履行发生争议。
...
本任务一般买卖转合同争议，审查起草搁置，不给胜诉率，不指导伪造、抢夺、威胁、隐匿病历或中断救治取证。

## 建立坐标

确认：使用者与授权；单方立场；中选通知、量价协议、供货数量、断供原因、质量投诉、医保或招采部门文件；法域；当前投诉、封存、鉴定、调解、监管或诉讼状态；机关文书作出与送达；对方已知主张；关键材料及缺失。
...
宿主可调用时：`search_law` 核验集中采购相关现行政策规则待核、民法典合同编、药品管理法供应和质量交叉；`search_practiced` 补路径与反例；特殊事实用 `search_case_by_fact`。不虚构鉴定、许可或案件系统。

工具不可用时继续事实、主体、材料和问题清单；集采特殊违约和监管后果法律结论字段停止。
