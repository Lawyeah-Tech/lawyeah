---
name: 识别重要数据
description: 识别重要数据状态并给出未告知时的出境接口
---
...
形成识别依据、未被告知或公布时的出境接口、年报红旗。

`depends-on` 为空。接受用户、律师、机关或其他来源的等价事实、处理记录和证据清单。

## 工作边界

正触发：要判断某类数据是否属于重要数据、要不要按重要数据处理。

负触发并输出 `触发判定：负触发，未启动本任务`：

- 未被告知或公布却要求写成已确定重要数据并强制出境评估；
- 只要分类分级目录；
- 只要核心数据义务；
- 出口管制物项认定；
...
确认：数据描述；是否已申报识别；是否已被告知或公布；行业目录；出境计划。
...
宿主可调用时：`search_law` 核验数安法、网络数据安全管理条例第 29、37 条及已公布目录。不虚构案件系统。

工具不可用时继续主体、场景、系统和问题清单；依赖未核验规则的法律结论、期限或机关路径字段停止。

## 五个判断节点

执行顺序和停止条件以 [重要数据识别方法](references/method.md) 为准。这里只列入口，不代替节点。

### 节点 1　识别状态

已公布／已告知／已申报未告知／未识别 四态必须标明。

### 节点 2　不得自行终局

未被告知或公布，不得写成已确定重要数据并强制出境评估。
...
条例第 37 条：按规定识别申报但未被告知或公布的，不需要作为重要数据申报出境安全评估。同时继续识别。

### 节点 5　年报红旗

重要数据处理者风险评估与年报义务只标接口，不代写年报。

## 停止与安全

- `整体停止`：在未告知未公布时写成已确定重要数据并强制评估，或写成已证明不是重要数据。
- `字段停止`：条例第 29、37 条及目录未核验。
- `目标停止`：主目标变为分类分级全目录或出口管制。
