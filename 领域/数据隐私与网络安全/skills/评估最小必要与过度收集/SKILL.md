---
name: 评估最小必要与过度收集
description: 对照目的判断是否过度收集并列出可拒收项
---
...
形成目的—种类—必要性矩阵、可拒收项、超范围红旗。不替代合法性基础判断。

`depends-on` 为空。接受用户、律师、机关或其他来源的等价事实、处理记录和证据清单。

## 工作边界

正触发：被指超范围收集、与目的无关或权限过多。
...
确认：宣称目的；实际收集种类与权限；基本功能与附加功能；拒绝某类信息后的功能影响。
...
宿主可调用时：`search_law` 核验第 5、6、19 条及 App 必要信息范围规定现行文本。不虚构案件系统。

工具不可用时继续主体、场景、系统和问题清单；依赖未核验规则的法律结论、期限或机关路径字段停止。

## 五个判断节点

执行顺序和停止条件以 [最小必要方法](references/method.md) 为准。这里只列入口，不代替节点。

### 节点 1　目的锁定

先写处理者宣称的目的，再写实际使用。目的空泛（“改善体验”）不得覆盖敏感或非必要种类。

### 节点 2　种类对照

身份、通讯录、定位、相册、设备信息等分列。未使用的种类标超收候选。

### 节点 3　基本功能拆分

基本功能所必需与附加功能分列。附加功能可拒。

### 节点 4　可拒收项

拒绝后仍能使用基本功能的种类列入可拒。频繁索要打开捆绑原子。

### 节点 5　结论

不替代第 13 条。超收成立不等于处理全部违法，按种类切割。

## 停止与安全

- `整体停止`：指导把非必要写成必要以规避拒收；伪造目的。
- `字段停止`：行业必要信息范围清单未核验。
- `目标停止`：主目标变为合法性总评、捆绑专项或 App 下架应对。
