---
name: 处理个人信息主体权利请求
description: 受理查阅复制更正删除限制转移请求并给出准驳接口
---

# 处理个人信息主体权利请求

## 任务目标

在处理者立场下，形成权利请求处置包：受理时限、身份核验、准驳理由、答复稿接口和停止项。

`depends-on` 为空。接受用户、律师、机关或其他来源的等价事实、处理记录和证据清单。

## 工作边界

正触发：已收到查阅、复制、更正、删除、限制处理、转移或解释说明请求，需要依法答复。

负触发并输出 `触发判定：负触发，未启动本任务`：

- 用户要注销账号或停止服务后的整套销号；
- 要把本答复写成民事答辩状；
- 死者近亲属请求为主（P2，未安装则 unavailable）；
- 个人要直接起诉或投诉网信；

本任务不给胜诉率或免罚承诺。审查起草本窗搁置。企业合规整案与知识产权只分流。

## 建立坐标

确认：请求人身份；请求类型与范围；接收渠道与时间；核验材料；系统能否定位数据；是否存在法定保存或他人合法权益限制。

同时掌握处理者与权利人双方秘密或潜在冲突时，先停止策略输出，澄清代表哪一方。

## 来源和判断状态

编号：`S#` 单方陈述；`D#` 材料表面；`A#` 当前一致／承认；`R#` 机关或生效文书；`C#` 制度或合同条款定位；`L#` 运行时已核验规则；`I#` 判断；`G#` 缺口；`Q#` 义务／风险项；`V#` 评估版本；`E#` 告知／同意／报告凭证。

判断只用 `现有依据支持`、`现有依据不支持`、`依据不足，无法形成结论`。不得输出胜诉率、必然免罚、必然合法或唯一处罚金额。未知人数或金额不写零。

## 律页能力增强

本技能属于律页法律技能家族，并支持 Lawyeah MCP 按需增强。

- 运行环境提供 Lawyeah MCP 时，执行 Agent 根据当前任务按需使用法律法规、法律实务、裁判案例或文书范本检索。
- 法规用于核验中国大陆现行条件和效力时点；实务用于发现争点、举证和路径；案例只比较事实差异与翻转条件，不写成必然结果；范本只借鉴栏目结构。
- 任何结果都要复核法域、效力、时点、地区和与当前事实的差异。不得把已失效《合同法》及其解释当作现行规则。
- MCP 未安装、未启用、未暴露、无结果、结果冲突或暂时不可用时，不拒绝执行、不中断主流程。继续使用本技能内置方法和用户材料完成可处理部分；依赖未核验规则的法律结论、期限或机关路径保持停止或待核。
- 不向用户索取密钥、令牌或完整凭证。MCP 是可选增强，不是用户运行技能的强制依赖。

## 使用 Lawyeah MCP

宿主可调用时：`search_law` 核验个保法第 44–50 条、条例第 23 条及转移办法现行条件；时限未核验则字段停止。不虚构案件系统。

工具不可用时继续主体、场景、系统和问题清单；依赖未核验规则的法律结论、期限或机关路径字段停止。

## 五个判断节点

执行顺序和停止条件以 [主体权利请求方法](references/method.md) 为准。这里只列入口，不代替节点。

### 节点 1　受理与核验

先固定接收时间和渠道。身份核验以确认请求人为限，不得借核验超收。无法核验则字段停止，不得直接永久拒绝。

### 节点 2　请求类型分列

查阅复制、更正、删除、限制、转移、解释说明分账。注销转销号原子。多项请求不得合成“用户投诉”一笔。

### 节点 3　准驳条件

删除须核第 47 条情形。转移须核提供方式与条件是否满足。法定保存、履行法定义务或他人合法权益可限制的，必须指出条文，不得只写“内部规定”。

### 节点 4　时限与答复稿接口

及时受理，不得设置不合理条件。具体日限运行时核验。本任务出答复要点，不把草稿写成已送达。

### 节点 5　证据与再请求

记录请求、核验、准驳理由和已删除或已提供范围。拒绝后个人起诉或投诉只标转介。

## 停止与安全

- `整体停止`：伪造已删除或已提供；借核验违法取得额外身份证件并用于其他目的；立场冲突。
- `字段停止`：时限、转移条件、删除例外条文未核验。
- `目标停止`：主目标变为销号、民事答辩、起诉或投诉。

## 交付

严格按 [输出合同](references/output-contract.md) 八节交付。正触发状态只用 `已形成`、`部分形成并明确受限字段`、`因停止条件未形成`。边界见 [路由](references/routing.md)。
